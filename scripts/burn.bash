...
format_sd_card() {
    ...
    if [ "$SIZE" -lt 3980000000 ]; then
        echo -e "\e[31mA 2GB sd card or bigger is required for this script to work\e[0m\n"
        exit 1
    fi
...
copy_uboot() {
    ...
    if [ ! -f ${SRCDIR}/uImage ]; then
        echo -e "File not found: ${SRCDIR}/uImage\n"
        exit 1
    fi

    if [ ! -f ${SRCDIR}/am335x-boneblack.dtb ]; then
        echo -e "File not found: ${SRCDIR}/am335x-boneblack.dtb\n"
        exit 1
    fi
...
copy_rootfs() {
    ...
    if [ -b $DEV ]; then
        ...
        echo "Extracting rootfs.tar.xz to /media/card"
        sudo tar -C /media/card -xJf ${SRCDIR}/rootfs.tar.xz
